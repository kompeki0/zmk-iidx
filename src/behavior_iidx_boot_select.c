...
static int on_pressed(struct zmk_behavior_binding *binding,
    ...
    data->pressed_binding =
        in_boot_window ? &config->mode_binding : &config->fallback_binding;

    LOG_DBG("Boot-select press at %lld ms: %s", (long long)event.timestamp,
            in_boot_window ? "IIDX mode" : "fallback behavior");
...
#define IIDX_BOOT_SELECT_INST(n)                                                                   \
    ...
    static const struct behavior_iidx_boot_select_config                                          \
        behavior_iidx_boot_select_config_##n = {                                                  \
