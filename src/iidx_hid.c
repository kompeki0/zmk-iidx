...
/*
 * Report ID: none
 *   byte 0: X, absolute, -127..127
 *   byte 1: Y, absolute, -127..127
 *   byte 2: buttons 1..8 (only 1..7 are exposed by this module)
 *   byte 3: buttons 9..16 (only 9..12 are exposed by this module)
 *   byte 4: constant padding
 */
...
int zmk_iidx_hid_send(void) {
    ...
    /* endpoint_sem also protects tx_report until the asynchronous write completes. */
    k_mutex_lock(&report_mutex, K_FOREVER);
